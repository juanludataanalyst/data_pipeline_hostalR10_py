...
room_reservations_confirmed <- room_reservations %>% 
  ...
  filter(id_name_type != 'TBC') %>% 
    mutate(id_name_type = ifelse(id_name_type == 'double e shared bathroom',
                               'double shared bathroom',
                               id_name_type
                               )) %>% ## Borrar cuando actualice
...
ggplot(occupation_rate_by_room, aes(media, occupation_rate, color = as.factor(id_name_type))) +  
  ...
  geom_point(size = 3) +  # Luego, añade los puntos
   geom_text(aes(label = paste0("RevPar ", round(RevPar, 0)," COP")), vjust = -1, size = 3.5) + 
...
Porcentaje de habitaciones ocupadas por dia 
...
Duracion de la estancia por cada tipo de habitacion
...
Ingresos por cada tipo de habitacion
