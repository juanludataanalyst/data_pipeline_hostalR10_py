...
Cleaning data and adjusting prices out of discount range
...
ocupacion_por_dia <- reservas_diarias %>%
 ...
 ggplot(ocupacion_por_dia %>% 
  ...
  filter(grepl("bathroom", id_name_type))
          , aes(x = fecha, y = ocupacion_porcentaje,color = id_name_type)) +
  theme_minimal() + 
  scale_y_continuous(limits = c(0, 1), labels = scales::percent)+
     geom_line() +
    
      facet_wrap(~ id_name_type, ncol = 2, scales = "free_y") +  # Ajusta el número de columnas si lo deseas
   labs(
         x = "Fecha",
